...
var TableExcelHelper = {
    createTableHead: function (columns, totalWidth) {
        ...
        for (var layer = 0; layer < tree.totalLayer; layer++) {
            ...
            tree[layer].forEach(function (col) {
                ...
                td.html(col.title);
                if (colWidths[col.id]) {
                    col.hasWidth = true; col.targetWidth = colWidths[col.id];
                    td.attr("haswidth", "haswidth"); td.css("width", colWidths[col.id] + "pt");
                } else { col.hasWidth = false; }
                tr.append(td);
            });
            thead.append(tr);
        }
    ...
    createTableRow: function (columns, data, options, printElementType) {
        ...
        if (printElementType.groupFields && printElementType.groupFields.length) {
            hinnn.groupBy(rows, printElementType.groupFields, function (row) {
                var key = {};
                printElementType.groupFields.forEach(function (f) { key[f] = row[f]; });
                return key;
    ...
    createRowTarget: function (tree, rowData, options, printElementType) {
        ...
        tree.rowColumns.forEach(function (col, idx) {
            ...
            var val = formatter ? formatter(rowData[col.field], rowData, idx, options) : rowData[col.field];
            td.html(val);
            var styler = TableExcelHelper.getColumnStyler(col);
            if (styler) {
                var style = styler(rowData[col.field], rowData, idx, options);
                if (style) Object.keys(style).forEach(function (k) { td.css(k, style[k]); });
            }
            tr.append(td);
        });
        ...
        if (rowStyler) {
            ...
            if (rs) Object.keys(rs).forEach(function (k) { tr.css(k, rs[k]); });
        }
        return tr;
    },
    ...
    allAutoWidth: function (tree) {
        var total = 0;
        tree.rowColumns.forEach(function (col) { total += col.fixed ? 0 : col.width; });
        return total;
    },
    allFixedWidth: function (tree) {
        var total = 0;
        tree.rowColumns.forEach(function (col) { total += col.fixed ? col.width : 0; });
        return total;
    },
    reconsitutionTableColumnTree: function (columns, existing, n) {
        ...
        for (var i = 0; i < columns.length; i++) {
            ...
            if (i === 0) {
                columns[i].columns.forEach(function (col) { tree.colspan += col.colspan; });
            }
        }
    ...
    syncTargetWidthToOption: function (columns) {
        columns.forEach(function (layer) {
            layer.columns.forEach(function (col) { if (col.hasWidth) col.width = col.targetWidth; });
        });
    },
    getFooterFormatter: function (options, pte) {
        ...
        if (options.footerFormatter) {
            try { fn = new Function("return (" + options.footerFormatter + ")")(); } catch (e) { console.log(e); }
        }
        return fn;
    },
    getRowStyler: function (options, pte) {
        ...
        if (options.rowStyler) {
            try { fn = new Function("return (" + options.rowStyler + ")")(); } catch (e) { console.log(e); }
        }
        return fn;
    },
    getColumnStyler: function (col) {
        ...
        if (col.styler2) {
            try { fn = new Function("return (" + col.styler2 + ")")(); } catch (e) { console.log(e); }
        }
        return fn;
    },
    getColumnFormatter: function (col) {
        ...
        if (col.formatter2) {
            try { fn = new Function("return (" + col.formatter2 + ")")(); } catch (e) { console.log(e); }
        }
        return fn;
    },
