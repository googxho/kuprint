...
import { IdCreator, InnerElementEditor, EditorFactory } from "../core/print-element-option.js";
...
TableColumn.prototype.getTarget = function () {
  return this.target;
};
TableColumn.prototype.getValue = function () {
...
TableColumn.prototype.isOverlap = function (r) {
  ...
  return (
    r.x + r.width > tr.x && tr.x + tr.width > r.x && r.y + r.height > tr.y && tr.y + tr.height > r.y
  );
};
TableColumn.prototype.isInRect = function (ctx) {
  var r = ctx.rect,
    tr = this.getTableRect();
  if (
    r.x + r.width > tr.x &&
    tr.x + tr.width > r.x &&
    r.y + r.height > tr.y &&
...
TableColumn.prototype.isSelected = function () {
  return this.target.hasClass("selected");
};
TableColumn.prototype.select = function () {
  this.target.addClass("selected");
};
TableColumn.prototype.isHeader = function () {
  return false;
};
...
TableColumn.prototype.getEntity = function () {
  return new TableColumnEntity(this);
};

export { TableColumnEntity, TableColumn };
