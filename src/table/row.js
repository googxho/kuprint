...
// 依赖：TableColumn（column.js）
// ============================================================

var TableColumnFull = (function (_super) {
    __extends(TableColumnFull, _super);
    function TableColumnFull(opts) {
        var self = _super.call(this) || this;
        opts = opts || {};
        self.width = opts.width ? parseFloat(opts.width.toString()) : 100;
        self.title = opts.title; self.descTitle = opts.descTitle;
        self.field = opts.field; self.fixed = opts.fixed;
        self.rowspan = opts.rowspan ? parseInt(opts.rowspan) : 1;
        self.colspan = opts.colspan ? parseInt(opts.colspan) : 1;
        self.align = opts.align; self.halign = opts.halign; self.vAlign = opts.vAlign;
        self.formatter = opts.formatter; self.styler = opts.styler;
        self.formatter2 = opts.formatter2; self.styler2 = opts.styler2;
        self.checkbox = opts.checkbox; self.checked = opts.checked != 0;
        self.columnId = opts.columnId || opts.field;
        return self;
    }
    TableColumnFull.prototype.css = function () { };
    return TableColumnFull;
})(TableColumn);
...
var RectWithChanged = function (rect) { this.rect = rect; };
var CellPosition = function (rowIndex, cell) { this.rowIndex = rowIndex; this.cell = cell; };

var HiCellSelector = (function () {
    function HiCellSelector(rows, tableTarget) {
        this.selectedCells = []; this.rows = rows; this.tableTarget = tableTarget;
    }
    ...
    HiCellSelector.prototype.setSingleSelect = function (cell) {
        this.startCell = cell; this.selectedCells = [];
    };
        ...
        if (this.startCell) {
            ...
            if (endCell) {
                var merged = RectHelper.mergeRect(
                    this.startCell.cell.getTableRect(), endCell.cell.getTableRect()
                );
    ...
    HiCellSelector.prototype.selectByRect = function (ctx, result) {
        ...
        this.rows.forEach(function (row, i) {
            ...
            row.columns.forEach(function (col) {
                if (col.isInRect(ctx)) { cells.push(new CellPosition(i, col)); col.select(); }
            });
            if (cells.length) result.push(cells);
        });
        if (ctx.changed) { ctx.changed = false; result.splice(0, result.length); self.selectByRect(ctx, result); }
    };
    HiCellSelector.prototype.getSelectedCells = function () { return this.selectedCells; };
    HiCellSelector.prototype.getCellByXY = function (x, y) {
        var found;
        this.rows.forEach(function (row, i) {
            var cols = row.columns.filter(function (col) { return col.isXYinCell(x, y); });
            if (cols.length) found = new CellPosition(i, cols[0]);
        });
        return found;
    };
    return HiCellSelector;
})();

// ============================================================
// Module 14: RectHelper
// ============================================================
var RectHelper = {
    ...
    Rect: function (x1, y1, x2, y2) {
        return {
            minX: x1 < x2 ? x1 : x2, minY: y1 < y2 ? y1 : y2,
            maxX: x1 < x2 ? x2 : x1, maxY: y1 < y2 ? y2 : y1
        };
    }
};

// ============================================================
// Module 6: PaperHtmlResult
// ============================================================
var PaperHtmlResult = function (opts) {
    this.printLine = opts.printLine; this.target = opts.target;
...
var PrintReferenceElement = (function () {
    function PrintReferenceElement(opts) {
        this.top = opts.top; this.left = opts.left;
        this.height = opts.height; this.width = opts.width;
...
// ============================================================
var TableRowBase = (function () {
    function TableRowBase() { this.id = IdCreator.createId(); }
    ...
    TableRowBase.prototype.getTarget = function () { return this.target; };
    ...
    TableRowBase.prototype.createTableCell = function (rowspan, colspan) {
        ...
        if (rowspan > 1) { cell.getTarget().attr("rowspan", rowspan); cell.rowspan = rowspan; }
        if (colspan > 1) { cell.getTarget().attr("colspan", colspan); cell.colspan = colspan; }
        return cell;
    };
    ...
    TableRowBase.prototype.insertCellToFirst = function (cell) {
        this.target.prepend(cell.getTarget()); this.columns.splice(0, 0, cell);
    };
    TableRowBase.prototype.insertCellToLast = function (cell) {
        this.columns.push(cell); this.target.append(cell.getTarget());
    };
    TableRowBase.prototype.getPrintElementOptionEntity = function () {
        var result = [];
        this.columns.forEach(function (col) { result.push(col.getEntity()); });
        return result;
    };
    return TableRowBase;
})();
...
// ============================================================
// Module 12: TableColumnRow
// ============================================================
var TableColumnRow = (function (_super) {
    __extends(TableColumnRow, _super);
    function TableColumnRow(opts) {
        var self = _super.call(this) || this;
        self.columns = [];
        if (opts && opts.constructor === Array) {
            (opts || []).forEach(function (c) { self.columns.push(new TableColumnFull(c)); });
        } else if (opts && opts.columns) {
            (opts.columns || []).forEach(function (c) { self.columns.push(new TableColumnFull(c)); });
        }
        return self;
    }
...
// ============================================================
// Module 19: ReconsitutionTableColumns
// ============================================================
var ReconsitutionTableColumns = function () { this.rowColumns = []; };

// ============================================================
// Module 20: GridColumnsStructure
// ============================================================
var GridColumnsStructure = (function () {
    function GridColumnsStructure(count, target) {
        this.gridColumns = count; this.target = target;
    }
    GridColumnsStructure.prototype.getByIndex = function (idx) {
        return this.target.find(".hi-grid-col:eq(" + idx + ")");
    };
    return GridColumnsStructure;
})();

// ============================================================
// Module 7: TableExcelHelper
// ============================================================