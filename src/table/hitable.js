// ============================================================
// table/hitable.js — HiTale 可编辑表格 + 元素类型D
// ============================================================
// HiTale 可编辑表格（行列增删/合并拆分/拖拽排序）+ 元素类型类
// 注意：各元素实现已拆分到 elements/ 目录
// ============================================================

var HiTaleOptions = function (opts) {
    this.table = opts.table; this.isEnableEdit = opts.isEnableEdit;
    this.trs = opts.trs; this.resizeRow = opts.resizeRow;
    this.resizeColumn = opts.resizeColumn; this.isEnableEditField = opts.isEnableEditField;
...
var HiTaleOptionsCoat = (function () {
    function HiTaleOptionsCoat(opts) { this.options = new HiTaleOptions(opts); }
    HiTaleOptionsCoat.prototype.enableEdit = function () { return this.options.isEnableEdit; };
    HiTaleOptionsCoat.prototype.disableEdit = function () { return this.options.isEnableEdit; };
    HiTaleOptionsCoat.prototype.isEnableEdit = function () { return this.options.isEnableEdit; };
    return HiTaleOptionsCoat;
})();

var CellGridItem = function (opts) {
    this.cell = opts.cell; this.link = opts.link; this.linkType = opts.linkType;
    this.bottom = opts.bottom; this.rightMost = opts.rightMost;
    this.rowLevel = opts.rowLevel; this.columnLevel = opts.columnLevel;
...
var GridHelper = {
    getLeftTableCell: function (gridRow, idx) {
        var result;
        gridRow.forEach(function (item, i) { if (item.cell && i < idx) result = item.cell; });
        return result;
    },
    getIndex: function (gridRow, cellId) {
        var result;
        gridRow.forEach(function (item, i) { if (item.cell && item.cell.id === cellId) result = i; });
        return result;
    }
};

var GripContainer = function (target, grips) { this.target = target; this.grips = grips; };
var RowGrip = function (target) { this.target = target; };
var ReconsitutionTableColumns2 = function () { this.rowColumns = []; };
...
var HiTaleColumnHelper = {
    ...
    allAutoWidth: function (tree) {
        var total = 0;
        tree.rowColumns.forEach(function (col) { total += col.width; });
        return total;
    },
    reconsitutionTableColumnTree: function (columns, existing) {
        ...
        for (var i = 0; i < columns.length; i++) {
            ...
            tree.rowColumns = tree.rowColumns.concat(
                tree[i].filter(function (col) { return col.rowspan === columns.length - i; })
            );
        }
        return tree;
    }
...
var HiTresizer = (function () {
    function HiTresizer(hitable) {
        ...
        this.hitable = hitable; this.rows = hitable.rows; this.target = hitable.target;
    }
    ...
    HiTresizer.prototype.resizeTableCellWidth = function () { HiTaleColumnHelper.resizeTableCellWeight(this.rows); };
    ...
    HiTresizer.prototype.updateColumnGrips = function () {
        if (this.cgripContainer) { this.cgripContainer.target.remove(); this.createColumnGrips(); }
    };
    HiTresizer.prototype.updateRowGrips = function () {
        if (this.rgripContainer) { this.rgripContainer.target.remove(); this.createRowGrips(); }
    };
    HiTresizer.prototype.createRowGrips = function () {
        ...
        this.rows.forEach(function (row, i) {
            ...
            if (i > 0 && i < self.rows.length) {
                $(grip).hidraggable({
                    axis: "v", onDrag: function () { }, moveUnit: "pt", minMove: 1,
                    onBeforeDrag: function () {
                        self.dragingGrip = gripObj;
                    ...
                    onStopDrag: function () {
                        ...
                        var h = hinnn.px.toPt(newTop - self.dragingGrip.top + self.rows[i].columns[0].getTarget().height());
    ...
    HiTresizer.prototype.syncGrips = function (cell, grip) {
        ...
        grip.cell = cell;
        grip.target.css({ left: target.offset().left - this.target.offset().left + target.outerWidth(false), height: 30 });
    };
    HiTresizer.prototype.syncRowGrips = function () {
        ...
        this.rows.forEach(function (row, i) {
            ...
            self.rgripContainer.grips[i].target.css({
                top: td.offset().top - self.target.offset().top + td.outerHeight(false), width: 30
            });
        });
    };
    HiTresizer.prototype.addResizerHeadRow = function () { this.target.find("thead").prepend(""); };
    return HiTresizer;
})();

var NoopResizer = { init: function () { }, updateRowGrips: function () { }, updateColumnGrips: function () { } };

var HiTale = (function () {
    function HiTale(opts) {
        this.id = IdCreator.createId();
        this.optionsCoat = new HiTaleOptionsCoat(opts);
        this.handle = opts.handle; this.target = opts.table;
        ...
        if (pos === "above") {
            grid[rowIdx].forEach(function (item) {
                ...
                if (item.columnLevel === 0) {
                    ...
                    c.width = w; newRow.insertCellToLast(c);
            ...
            grid[bottomIdx].forEach(function (item) {
                ...
                if (item.bottom) {
                    ...
                    if (item.cell) { item.cell.rowspan += 1; item.cell.getTarget().attr("rowspan", item.cell.rowspan); }
                    if (item.linkType === "column") { item.link.rowspan += 1; item.link.getTarget().attr("rowspan", item.link.rowspan); }
                }
            });
    ...
    HiTale.prototype.insertColumn = function (pos, cellPos, cls, width) {
        ...
        if (pos === "left") {
            ...
            grid.forEach(function (row, i) {
                ...
                var rightCells = row.filter(function (it, idx) { return idx >= gridCol && it.cell; });
                if (item.rowLevel === 0) {
                    var tr = allRows[i];
            ...
            grid.forEach(function (row, i) {
                ...
                var leftCells = row.filter(function (it, idx) { return idx <= rightGridCol && it.cell; });
                if (item.rightMost) {
                    var tr = allRows[i];
                    var c = tr.createTableCell();
                    if (cls) c.getTarget().addClass(cls);
                    ...
                    if (item.linkType === "row") { item.link.colspan += 1; item.link.getTarget().attr("colspan", item.link.colspan); }
                    if (item.cell) { item.cell.colspan += 1; item.cell.getTarget().attr("colspan", item.cell.colspan); }
                }
            });
        }
    };
        ...
        grid[rowIdx].forEach(function (item, colIdx) {
            if (item.cell) {
                if (item.cell.rowspan === 1) {
                    ...
                    row.removeCell(item.cell);
                    var nextCells = grid[rowIdx + 1].filter(function (it, i) { return it.cell && i > colIdx; });
    ...
    HiTale.prototype.deleteColums = function () {
        ...
        grid.forEach(function (row, i) {
            ...
            if (item.cell) {
                ...
                else { item.cell.colspan -= 1; item.cell.getTarget().attr("colspan", item.cell.colspan); }
            } else if (item.linkType === "row") {
                item.link.colspan -= 1; item.link.getTarget().attr("colspan", item.link.colspan);
            }
        });
    };
    HiTale.prototype.mergeCell = function () {
        ...
        cells.forEach(function (row, r) {
            row.forEach(function (pos, c) {
                if (r === 0) {
                    if (c !== 0) { first.colspan += pos.cell.colspan; self.rows[pos.rowIndex].removeCell(pos.cell); }
                } else {
    ...
    HiTale.prototype.init = function (opts) {
        ...
        $(this.target).mousedown(function () { self.optionsCoat.isLeftMouseButtonDown = true; });
        $(this.target).mouseup(function () { self.optionsCoat.isLeftMouseButtonDown = false; });
        this.initContext();
    ...
    HiTale.prototype.initRows = function (rows) {
        ...
        if (rows) {
            ...
            if (trs) {
                this.initRowsByTrs(trs).forEach(function (r) { self.trRows.push(r); });
            }
        } else {
    ...
    HiTale.prototype.enableEdit = function () { this.optionsCoat.enableEdit(); };
    HiTale.prototype.disableEdit = function () { this.optionsCoat.disableEdit(); };
    ...
    HiTale.prototype.getColumnStep = function (rowIdx) {
        var total = 0;
        if (this.rows.length) {
            this.rows[rowIdx || 0].columns.forEach(function (col) { total += col.colspan; });
        }
        return total;
    };
    HiTale.prototype.initContext = function () {
        ...
        $(this.handle).hicontextMenu({
            menus: [
                {
                    text: "在上方插入行", enabled: this.optionsCoat.options.isEnableInsertRow,
                    disable: function () { return !self.tableCellSelector.getSingleSelect(); },
                    callback: function () { self.insertRow("above"); self.resizer.updateRowGrips(); hinnn.event.trigger("updateTable" + self.id); }
                },
                {
                    text: "在下方插入行", borderBottom: true, enabled: this.optionsCoat.options.isEnableInsertRow,
                    disable: function () { return !self.tableCellSelector.getSingleSelect(); },
                    callback: function () { self.insertRow("below"); self.resizer.updateRowGrips(); hinnn.event.trigger("updateTable" + self.id); }
                },
                {
                    text: "向左方插入列", enabled: this.optionsCoat.options.isEnableInsertColumn,
                    disable: function () { return !self.tableCellSelector.getSingleSelect(); },
                    callback: function () { self.insertColumn("left"); self.resizer.updateColumnGrips(); hinnn.event.trigger("updateTable" + self.id); }
                },
                {
                    text: "向右方插入列", enabled: this.optionsCoat.options.isEnableInsertColumn, borderBottom: true,
                    disable: function () { return !self.tableCellSelector.getSingleSelect(); },
                    callback: function () { self.insertColumn("right"); self.resizer.updateColumnGrips(); hinnn.event.trigger("updateTable" + self.id); }
                },
                {
                    text: "删除行", enabled: this.optionsCoat.options.isEnableDeleteRow,
                    disable: function () { return !self.tableCellSelector.getSingleSelect(); },
                    callback: function () { self.deleteRow(); self.resizer.updateRowGrips(); hinnn.event.trigger("updateTable" + self.id); }
                },
                {
                    text: "删除列", borderBottom: true, enabled: this.optionsCoat.options.isEnableDeleteColumn,
                    disable: function () { return !self.tableCellSelector.getSingleSelect(); },
                    callback: function () { self.deleteColums(); self.resizer.updateColumnGrips(); hinnn.event.trigger("updateTable" + self.id); }
                },
                {
                    text: "对齐", borderBottom: true, enabled: this.optionsCoat.options.columnAlignEditable,
                    menus: [
                        { text: "左", callback: function () { self.setAlign("left"); } },
                        { text: "左右居中", callback: function () { self.setAlign("center"); } },
                        { text: "右", callback: function () { self.setAlign("right"); } },
                        { text: "默认", borderBottom: true, callback: function () { self.setAlign(""); } },
                        { text: "上", callback: function () { self.setVAlign("top"); } },
                        { text: "垂直居中", callback: function () { self.setVAlign("middle"); } },
                        { text: "下", callback: function () { self.setVAlign("bottom"); } },
                        { text: "默认", callback: function () { self.setVAlign(""); } }
                    ]
                },
                {
                    text: "合并单元格", enabled: this.optionsCoat.options.isEnableMergeCell,
                    disable: function () { return self.tableCellSelector.getSingleSelect(); },
                    callback: function () { self.mergeCell(); hinnn.event.trigger("updateTable" + self.id); }
                },
                {
                    text: "解开单元格", enabled: this.optionsCoat.options.isEnableMergeCell,
                    disable: function () {
                        var pos = self.tableCellSelector.getSingleSelect();
                        return !pos || (pos.cell.rowspan === 1 && pos.cell.colspan === 1);
                    },
                    callback: function () { self.splitCell(); hinnn.event.trigger("updateTable" + self.id); }
                }
            ].filter(function (m) { return m.enabled; })
        });
    };
    HiTale.prototype.getTableWidth = function () { return hinnn.px.toPt(this.target.outerWidth(false)); };
    HiTale.prototype.updateColumnGrips = function () { this.resizer.updateColumnGrips(); };
    HiTale.prototype.updateRowGrips = function () { this.resizer.updateRowGrips(); };
    return HiTale;
})();
