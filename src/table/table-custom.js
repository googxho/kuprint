// ============================================================
// elements/table-custom.js — 自定义表格元素
// TableCustomPrintElement 完全手动设计表头
// ============================================================
...
var TableCustomPrintElement = (function (_super) {
    ...
    function TableCustomPrintElement(pte, opts) {
        ...
        self.options.setDefault(new TableCustomPrintElementOption(KuPrintConfig.instance.tableCustom.default).getPrintElementOptionEntity());
    ...
    TableCustomPrintElement.prototype.getConfigOptions = function () { return KuPrintConfig.instance.tableCustom; };
    TableCustomPrintElement.prototype.createTarget = function (title, data, n) {
        var $el = $('<div class="kuprint-printElement kuprint-printElement-table" style="position:absolute;">' +
    ...
    TableCustomPrintElement.prototype.getTableHtml = function (data, templateData) {
        var table = $('<table class="kuprint-printElement-tableTarget" style="border-collapse:collapse;width:100%;"></table>');
        ...
        if (footerFn) {
            if (this.options.tableFooterRepeat === "no") { }
                ...
                table.find("tbody").append(TableExcelHelper.createTableFooter(
                    this.printElementType.columns, data, this.options, this.printElementType, templateData, data
                ).html());
            } else {
                table.append(TableExcelHelper.createTableFooter(
                    this.printElementType.columns, data, this.options, this.printElementType, templateData, []
                ));
            }
        }
        return table;
    ...
    TableCustomPrintElement.prototype.getPaperHtmlResult = function (paper, data) {
        ...
        var pageIdx = 0, isEnd = false, anchorTop;
        while (!isEnd) {
            var extraHeight = 0;
            ...
            var h = extraHeight > 0 ? extraHeight : pageIdx === 0 ? footer - printTop : paper.getContentHeight(pageIdx);
            var rowsResult = this.getRowsInSpecificHeight(h, emptyTarget, tableHtml, pageIdx, prevTarget, data);
            isEnd = rowsResult.isEnd;
            var printLine;
            if (rowsResult.target) { rowsResult.target.css("left", this.options.displayLeft()); rowsResult.target[0].height = ""; }
            ...
            results.push(new PaperHtmlResult({
                target: rowsResult.target, printLine: printLine,
                referenceElement: new PrintReferenceElement({
                    ...
                    bottomInLastPaper: printLine, printTopInPaper: anchorTop
                })
            }));
            pageIdx++;
        }
    ...
    TableCustomPrintElement.prototype.getRowsInSpecificHeight = function (availHeight, emptyTarget, tableHtml, pageIdx, prevTarget, data) {
        ...
        while (true) {
            if (currentHeight <= availPx) {
                if (sourceTbody.find("tr").length === 0) {
                    if (prevTarget && this.options.autoCompletion) { this.autoCompletion(availPx, emptyTarget); currentHeight = emptyTarget.outerHeight(); }
                    result = { target: emptyTarget.clone(), length: emptyTarget.find("tbody tr").length, height: hinnn.px.toPt(currentHeight), isEnd: true };
                    if (emptyTarget.find("tbody tr").length === 0 && prevTarget) { result = { target: undefined, length: 0, height: 0, isEnd: true }; }
                } else {
                    ...
                    if (currentHeight > availPx) {
                        sourceTbody.prepend(firstRow); consumedRows.pop();
                        currentHeight = emptyTarget.outerHeight();
            ...
            if (result) {
                if (this.printElementType.footerFormatter && emptyTarget.find("tfoot") && result.target) {
                    result.target.find("tfoot").html(TableExcelHelper.createTableFooter(
                        this.printElementType.columns, this.getData(data), this.options, this.printElementType, data, consumedRows
                    ).html());
                }
                break;
            }
    ...
    TableCustomPrintElement.prototype.autoCompletion = function (availPx, table) {
        ...
        var lastRow;
        while (availPx > height) { lastRow = emptyRow.clone(); table.find("tbody").append(lastRow); height = table.outerHeight(); }
        if (lastRow) lastRow.remove();
    };
    TableCustomPrintElement.prototype.getEmptyRowTarget = function () { return TableExcelHelper.createEmptyRowTarget(this.columns); };
    ...
    TableCustomPrintElement.prototype.getReizeableShowPoints = function () { return ["s", "e"]; };
    TableCustomPrintElement.prototype.design = function (opts, paper) {
        ...
        this.designTarget.hireizeable({
            ...
            onResize: function (e, h, w, t, l) { self.onResize(e, h, w, t, l); self.hitable.updateColumnGrips(); self.createLineOfPosition(paper); },
    ...
    TableCustomPrintElement.prototype.setHiResizeable = function () {
        ...
        this.hitable = new HiTale({
            table: this.designTarget.find("table"), rows: this.columns,
            resizeRow: false, resizeColumn: true,
            trs: $(this.designTarget).find("tbody tr"),
            handle: this.designTarget.find("table thead"),
            columnDisplayEditable: true, columnDisplayIndexEditable: true,
            columnResizable: true, columnAlignEditable: true,
            isEnableEdit: true, isEnableEditText: true, isEnableEditField: true,
            isEnableContextMenu: true, isEnableInsertRow: true, isEnableDeleteRow: true,
