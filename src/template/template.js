...
import { PrintPanel } from "../panel/panel.js";
...
var PrintTemplate = (function () {
  function PrintTemplate(opts) {
    ...
    if (cfg.template) {
      tmpl.panels.forEach(function (p) {
        self.printPanels.push(new PrintPanel(p, self.id));
      });
    }
      ...
      data.forEach(function (d) {
        if (d)
          self.printPanels.forEach(function (panel) {
            $ct.append(panel.getHtml(d, opts));
          });
      });
    } else {
      this.printPanels.forEach(function (panel) {
        $ct.append(panel.getHtml(data, opts));
      });
    }
  ...
  PrintTemplate.prototype.getJointHtml = function (data, opts, jointOpts) {
    ...
    var panels = [];
    this.printPanels.forEach(function (panel) {
      $ct.append(panel.getHtml(data, opts, panels, undefined, jointOpts));
    });
    return $ct;
  };
  ...
  PrintTemplate.prototype.getPaneltotal = function () {
    return this.printPanels.length;
  };
  PrintTemplate.prototype.createDefaultPanel = function () {
    return new PrintPanel(
      new PanelEntity({ index: this.printPanels.length, paperType: "A4" }),
      this.id,
    );
  };
  PrintTemplate.prototype.createContainer = function (container) {
    if (container) {
      ...
      this.container.addClass("kuprint-printTemplate");
    } else {
  ...
  PrintTemplate.prototype.getPrintElementSelectEventKey = function () {
    return "PrintElementSelectEventKey_" + this.id;
  };
  PrintTemplate.prototype.getBuildCustomOptionSettingEventKey = function () {
  ...
  PrintTemplate.prototype.getPaperType = function (idx) {
    if (idx == null) idx = 0;
  ...
  PrintTemplate.prototype.getPrintStyle = function (idx) {
    return this.printPanels[idx].getPrintStyle();
  };
  ...
  PrintTemplate.prototype.deletePrintElement = function (pe) {
    this.printPanels.forEach(function (panel) {
      panel.deletePrintElement(pe);
    });
  };
  ...
  PrintTemplate.prototype.removeTempContainer = function () {
    $(".kuprint_temp_Container").remove();
  };
  PrintTemplate.prototype.getTempContainer = function () {
  ...
  PrintTemplate.prototype.on = function (event, fn) {
  ...
  PrintTemplate.prototype.getPanel = function (idx) {
    if (idx == null) idx = 0;
    return this.printPanels[idx];
  };
  PrintTemplate.prototype.loadAllImages = function ($html, cb, attempt) {
    ...
    for (var i = 0; i < imgs.length; i++) {
      ...
      if (img.src && img.src !== window.location.href && img.src.indexOf("base64") === -1) {
        if (!(img && img.naturalWidth !== undefined && img.naturalWidth !== 0 && img.complete)) {
          allLoaded = false;
        }
      }
    }
    ...
    if (!allLoaded && attempt < 10) {
      setTimeout(function () {
        self.loadAllImages($html, cb, attempt);
      }, 500);
    } else {
      cb();
    }
  };
  PrintTemplate.prototype.setFields = function (fields) {
    this.fields = fields;
  };
  PrintTemplate.prototype.getFields = function () {
    return this.fields;
  };
