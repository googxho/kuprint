...
var LongTextPrintElement = (function (_super) {
    __extends(LongTextPrintElement, _super);
    function LongTextPrintElement(pte, opts) {
        var self = _super.call(this, pte) || this;
        self.options = new LongTextPrintElementOption(opts);
        self.options.setDefault(new LongTextPrintElementOption(KuPrintConfig.instance.longText.default).getPrintElementOptionEntity());
        return self;
    }
    LongTextPrintElement.prototype.getDesignTarget = function (paper) {
        var target = _super.prototype.getDesignTarget.call(this, paper);
    ...
    LongTextPrintElement.prototype.updateDesignViewFromOptions = function () {
        if (this.designTarget) {
            ...
            this.designTarget.find(".kuprint-printElement-longText-content").html(
                html.find(".kuprint-printElement-longText-content").html()
            );
            this.css(this.designTarget, data);
        }
    };
    LongTextPrintElement.prototype.getConfigOptions = function () { return KuPrintConfig.instance.longText; };
    LongTextPrintElement.prototype.getTitle = function () { return this.options.title || this.printElementType.title; };
    LongTextPrintElement.prototype.getData = function (data) {
        return data ? (data[this.getField()] || "") : (this.options.testData || this.printElementType.getData() || "");
    };
    ...
    LongTextPrintElement.prototype.createTarget = function (title, data) {
        var $el = $('<div class="kuprint-printElement kuprint-printElement-longText" style="position:absolute;">' +
    ...
    LongTextPrintElement.prototype.getText = function (title, data) {
        ...
        if (data) data = this.options.leftSpaceRemoved !== false ? data.toString().replace(/^\s*/, "") : data;
        var result = "";
        if (this.getField()) {
            result = (this.options.getHideTitle() ? "" : title ? title + "：" : "") +
                (formatter ? formatter(title, data, this.options, this._currenttemplateData) : data);
        } else {
            result = formatter ? formatter(title, title, this.options, this._currenttemplateData) : (title || "");
        }
        return result || "";
    };
    ...
    LongTextPrintElement.prototype.getPaperHtmlResult = function (paper, data, n) {
        ...
        lines.forEach(function (line, i) {
            var cleaned = self.options.leftSpaceRemoved !== false ? (line || "").toString().replace(/^\s*/, "") : line;
        ...
        while (chars.length > 0) {
            ...
            if (pageIdx === 0 || extraHeight > 0) {
                ...
                printLine = chars.length > 0 ? printTop + chunk.height :
                    (this.options.lHeight != null ? printTop + (chunk.height > this.options.lHeight ? chunk.height : this.options.lHeight) : printTop + chunk.height);
            } else {
            ...
            results.push(new PaperHtmlResult({
                target: chunk.target, printLine: printLine,
                referenceElement: new PrintReferenceElement({
                    top: this.options.getTop(), left: this.options.getLeft(),
                    height: this.options.getHeight(), width: this.options.getWidth(),
                    beginPrintPaperIndex: paper.index,
    ...
    LongTextPrintElement.prototype.getStringBySpecificHeight = function (chars, availHeight, dummyTarget) {
        var availPx = hinnn.pt.toPx(availHeight);
        var result = this.IsPaginationIndex(chars, chars.length - 1, availPx, dummyTarget);
        return result.IsPagination ? result : this.BinarySearch(chars, 0, chars.length - 1, availPx, dummyTarget);
    };
    ...
    LongTextPrintElement.prototype.IsPaginationIndex = function (chars, idx, availPx, dummyTarget) {
        ...
        if (idx >= chars.length - 1 && h1 < availPx) {
            return { IsPagination: true, height: hinnn.px.toPt(h1), length: chars.length, target: dummyTarget.clone() };
        }
