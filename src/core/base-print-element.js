...
//
// 依赖：hinnn、KuPrintlib、KuPrintConfig、PrintElementOptionItemManager
// ============================================================

var BasePrintElement = (function () {
    ...
    BasePrintElement.prototype.setTemplateId = function (id) { this.templateId = id; };
    BasePrintElement.prototype.setPanel = function (panel) { this.panel = panel; };
    BasePrintElement.prototype.getField = function () {
        return this.options.field || this.printElementType.field;
    };
    BasePrintElement.prototype.getTitle = function () { return this.printElementType.title; };
    ...
    BasePrintElement.prototype.getPrintElementEntity = function (withTid) {
        if (withTid) {
            return new PrintElementEntity(undefined, this.options.getPrintElementOptionEntity(),
                this.printElementType.getPrintElementTypeEntity());
        }
        return new PrintElementEntity(this.printElementType.tid, this.options.getPrintElementOptionEntity());
    };
    BasePrintElement.prototype.submitOption = function () {
        ...
        this.getPrintElementOptionItems().forEach(function (item) {
            ...
            if (val && typeof val === "object") {
                Object.keys(val).forEach(function (k) { self.options[k] = val[k]; });
            } else {
    ...
    BasePrintElement.prototype.getReizeableShowPoints = function () { return ["s", "e"]; };
    BasePrintElement.prototype.onResize = function (e, h, w, t, l) {
        this.updateSizeAndPositionOptions(l, t, w, h);
    };
    BasePrintElement.prototype.getOrderIndex = function () { return this.options.getTop(); };
    ...
    BasePrintElement.prototype.stylerCss = function ($el, data) {
        ...
        if (styler) {
            ...
            if (style) {
                Object.keys(style).forEach(function (k) { $el.css(k, style[k]); });
            }
        }
    };
    ...
    BasePrintElement.prototype.getPrintElementOptionItems = function () {
        ...
        if (configOpts) {
            var supportOpts = configOpts.supportOptions;
            if (supportOpts) {
                supportOpts.filter(function (o) { return !o.hidden; }).forEach(function (o) {
    ...
    BasePrintElement.prototype.filterOptionItems = function (items) {
        if (this.printElementType.field) {
            return items.filter(function (item) { return item.name !== "field"; });
        }
        return items;
    };
    BasePrintElement.prototype.createTempContainer = function () {
        this.removeTempContainer();
        $("body").append($('<div class="kuprint_temp_Container kuprint-printPaper" style="overflow:hidden;height:0px;box-sizing:border-box;"></div>'));
    };
    BasePrintElement.prototype.removeTempContainer = function () { $(".kuprint_temp_Container").remove(); };
    BasePrintElement.prototype.getTempContainer = function () { return $(".kuprint_temp_Container"); };
    BasePrintElement.prototype.isHeaderOrFooter = function () {
        return this.options.getTopInDesign() < this.panel.paperHeader ||
    ...
    BasePrintElement.prototype.setCurrenttemplateData = function (data) { this._currenttemplateData = data; };
    BasePrintElement.prototype.isFixed = function () { return this.options.fixed; };
    ...
    BasePrintElement.prototype.createLineOfPosition = function (paper) {
        ...
        } else { topLine.css("top", this.options.displayTop()); }
        if (!leftLine.length) {
            leftLine = $('<div class="leftlineOfPosition' + this.id + '" style="border:0;border-left:1px dashed rgb(169,169,169);position:absolute;height:100%;"></div>');
        ...
        } else { leftLine.css("left", this.options.displayLeft()); }
        if (!rightLine.length) {
            rightLine = $('<div class="rightlineOfPosition' + this.id + '" style="border:0;border-left:1px dashed rgb(169,169,169);position:absolute;height:100%;"></div>');
        ...
        } else { rightLine.css("left", this.options.getLeft() + this.options.getWidth() + "pt"); }
        if (!bottomLine.length) {
            bottomLine = $('<div class="bottomlineOfPosition' + this.id + '" style="border:0;border-top:1px dashed rgb(169,169,169);position:absolute;width:100%;"></div>');
        ...
        } else { bottomLine.css("top", this.options.getTop() + this.options.getHeight() + "pt"); }
    };
    ...
    BasePrintElement.prototype.getFormatter = function () {
        ...
        if (this.options.formatter) {
            try { fn = new Function("return (" + this.options.formatter + ")")(); } catch (e) { console.log(e); }
        }
        return fn;
    };
    BasePrintElement.prototype.getStyler = function () {
        ...
        if (this.options.styler) {
            try { fn = new Function("return (" + this.options.styler + ")")(); } catch (e) { console.log(e); }
        }
        return fn;
    };
    ...
    BasePrintElement.prototype.inRect = function (rect) {
        ...
        return rect.minX < off.left && rect.minY < off.top &&
            rect.maxX > off.left && rect.maxY > off.top;
    };
...
// ============================================================
// Module 17: PrintElementEntity
// ============================================================
var PrintElementEntity = function (tid, options, printElementType) {
    this.tid = tid; this.options = options; this.printElementType = printElementType;
};
