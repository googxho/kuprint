...
import { PrintElementTypeEntity, TablePrintElementType } from "../table/table-element-type.js";
import { TextPrintElement } from "../elements/text.js";
import { ImagePrintElement } from "../elements/image.js";
import { LongTextPrintElement } from "../elements/longtext.js";
import { HtmlPrintElement } from "../elements/html.js";
import {
  VlinePrintElement,
  HlinePrintElement,
  RectPrintElement,
  OvalPrintElement,
} from "../elements/lines.js";
...
var BasePrintElementType = (function () {
  ...
  BasePrintElementType.prototype.getData = function () {
    return this.data;
  };
  ...
  BasePrintElementType.prototype.getFields = function () {
    return this.fields;
  };
  BasePrintElementType.prototype.getOptions = function () {
    return this.options || {};
  };
...
var TableCustomPrintElementType = (function (_super) {
  ...
  function TableCustomPrintElementType(opts) {
...
var SimplePrintElementType = (function (_super) {
  ...
  function SimplePrintElementType(opts) {
    return _super.call(this, opts) || this;
  }
...
var ElementTypeManager = (function () {
  ...
  ElementTypeManager.prototype.getElementType = function (tid) {
    var matches = this.allElementTypes.filter(function (et) {
      return et.tid === tid;
    });
    return matches.length > 0 ? matches[0] : undefined;
  };
  ElementTypeManager.prototype.formatterModule = function (key) {
    return key || "_default";
  };
...
var PrintElementTypeGroup = function (name, types) {
  ...
  types.forEach(function (t) {
    self.printElementTypes.push(PrintElementTypeCreator.createPrintElementType(t));
  });
};
...
PrintElementTypeManager.build = function (container, moduleKey) {
  var key = PrintElementTypeManager.formatterModule(moduleKey);
  var draggableItems = new ElementTypeHtmlBuilder().createPrintElementTypeHtml(
    container,
    PrintElementTypeManager.getElementTypeGroups(key),
  );
  PrintElementTypeManager.enableDrag(draggableItems);
};
PrintElementTypeManager.buildByHtml = function (items) {
  PrintElementTypeManager.enableDrag(items);
};
...
PrintElementTypeManager.formatterModule = function (key) {
  return key || "_default";
};
  ...
  ElementTypeHtmlBuilder.prototype.createPrintElementTypeHtml = function (container, groups) {
    ...
    groups.forEach(function (group) {
      ...
      group.printElementTypes.forEach(function (et) {
        $subUl.append(
          '<li><a class="ep-draggable-item" tid="' + et.tid + '">  ' + et.getText() + " </a></li>",
        );
      });
      $ul.append($li);
    });
...
var PanelEntity = function (opts) {
  ...
  this.paperNumberLeft = opts.paperNumberLeft;
  this.paperNumberTop = opts.paperNumberTop;
  this.paperNumberDisabled = opts.paperNumberDisabled;
  this.paperNumberFormat = opts.paperNumberFormat;
  this.panelPaperRule = opts.panelPaperRule;
  this.rotate = opts.rotate || undefined;
  this.firstPaperFooter = opts.firstPaperFooter;
  this.evenPaperFooter = opts.evenPaperFooter;
  this.oddPaperFooter = opts.oddPaperFooter;
  this.lastPaperFooter = opts.lastPaperFooter;
  this.topOffset = opts.topOffset;
  this.fontFamily = opts.fontFamily;
  this.leftOffset = opts.leftOffset;
...
export {
  ...
  ElementTypeHtmlBuilder,
  BasePrintElementType,
  SimplePrintElementType,
  TableCustomPrintElementType,
  PrintElementFactory,
  PanelEntity,
  MouseRect,
};
